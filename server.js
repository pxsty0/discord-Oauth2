...
const {CLIENT_ID, CLIENT_SECRET, DEVELOPMENT} = require('./ayarlar.json');
const port = 8080;
...
app.use(session({secret: CLIENT_SECRET, resave: false, saveUninitialized: true, cookie: {expires: 2.16e+7}}));

app.use(express.static('public'));

const REDIRECT_URI = DEVELOPMENT ? `projeurlniz/callback`: "projeurlniz";
...
app.get("/cikis", async (req, res, next) => {
...
app.get("/callback", async (req, res, next) => {
    ...
    if (!code) return next(new Error("CODE YOK"));
